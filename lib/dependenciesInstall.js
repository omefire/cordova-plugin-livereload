...
module.exports = function(context) {
    ...
    return Q.promise(function(resolve, reject, notify) {
        npm.load({
            ...
            if (err) {
                console.log(pluginName + ': An error occurred while loading npm');
                reject(pluginName + ': npm load failed.');
                return;
            }
	    ...
	    // Install the dependencies from package.json
            npm.commands.install(packageJSONDir, [], function(er, data) { 
                ...
                if (err) {
                    console.log(pluginName + ': dependencies install failed.');
                    reject(pluginName + ': dependencies install failed.');
                    return;
                }
                resolve();
            });
