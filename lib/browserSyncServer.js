...
BrowserSyncServer.prototype.startServer = function () {
    ...
    self.browserSync.init(self.options, function (err, bs) {
        ...
        // get back all the urls 
        // what if the machine is connected to NO network ?
        // what to do if --tunnel ? what if not ?
        
        // Note: There are multiple possible urls: 'localhost', 'external', 'tunnel'.
        // Let's return all the urls instead of just the external url
        // ... because the iOS simulator can only to the 'localhost' url
